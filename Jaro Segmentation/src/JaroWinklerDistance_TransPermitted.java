 ...
 */
public class JaroWinklerDistance_TransPermitted implements StringDistance {

	// prefix scaling을 고려할지 안 할지! 
	// 음수면 그냥 Jaro
	...
	private int[] matches(String s1, String s2) {
		...
		// transposition 가능하게 하는 범위
		int range = max.length(); //Math.max(max.length() / 2 - 1, 0);
...
//		return new int[] { matches, transpositions / 2, prefix, max.length() };
		return new int[] { matches, 0, prefix, max.length() };
	}
	...
	public boolean equals(Object obj) {
		...
		JaroWinklerDistance_TransPermitted o = (JaroWinklerDistance_TransPermitted)obj;
